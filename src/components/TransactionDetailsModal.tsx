                    ...
                    <div className="flex justify-between items-start">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-black uppercase tracking-widest">Receipt: {sale.receiptId}</p>
                            </div>
                        </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-card">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div className="space-y-4">
                            <div className="bg-background rounded-2xl p-4 border border-card-border">
                                <p className="text-[10px] font-black text-black uppercase tracking-widest mb-1">Receipt ID</p>
                            ...
                            <div className="bg-background rounded-2xl p-4 border border-card-border">
                                <p className="text-[10px] font-black text-black uppercase tracking-widest mb-2">Payment Method</p>
                        ...
                        <div className="space-y-4">
                            <div className="bg-background rounded-2xl p-4 border border-card-border h-full flex flex-col justify-center">
                                <div>
                                    <p className="text-[10px] font-black text-black uppercase tracking-widest mb-1">Customer Name</p>
                                ...
                                {typeof sale.customer === 'object' && sale.customer && (
                                    <div className="mt-4 space-y-3 pt-4 border-t border-card-border">
                                        <div>
                                            <p className="text-[10px] font-black text-black uppercase tracking-widest mb-1">Phone</p>
                                            <p className="text-xs font-bold text-black">{sale.customer.phone || 'N/A'}</p>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-black uppercase tracking-widest mb-1">Address</p>
                                            <p className="text-xs font-bold text-black truncate" title={sale.customer.address}>{sale.customer.address || 'N/A'}</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="rounded-2xl border border-card-border overflow-x-auto shadow-sm">
                                <table className="min-w-full divide-y divide-card-border">
                                    <thead className="bg-sidebar">
                                        <tr>
                                            <th className="px-5 py-4 text-left text-[10px] font-black text-black uppercase tracking-widest">Item</th>
                                            <th className="px-5 py-4 text-center text-[10px] font-black text-black uppercase tracking-widest">Qty</th>
                                            <th className="px-5 py-4 text-right text-[10px] font-black text-black uppercase tracking-widest">Price</th>
                                            <th className="px-5 py-4 text-right text-[10px] font-black text-black uppercase tracking-widest">Total</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-card divide-y divide-card-border">
                                        {sale.cartItems.map((item, index) => (
                                            <tr key={index} className="hover:bg-pos-accent/5 transition-colors">
                                                ...
                                                <td className="px-5 py-4 text-xs font-bold text-black text-center bg-background/50 border-x border-card-border">{item.quantity}</td>
                                                <td className="px-5 py-4 text-xs font-bold text-black text-right">Rs. {item.price.toLocaleString()}</td>
                    ...
                    <div className="bg-background p-6 rounded-[24px] border border-card-border flex flex-col gap-4 relative overflow-hidden">
                        ...
                        {(sale.previousDues ?? 0) < 0 && (
                            <div className="flex justify-between items-center text-sm relative z-10">
                                <span className="text-[11px] font-black text-black uppercase tracking-widest">Previous Dues:</span>
                        ...
                        <div className="flex justify-between items-center border-t border-card-border pt-4 mt-2 relative z-10">
                            <span className="text-[11px] font-black text-black uppercase tracking-widest">Remaining Balance Due:</span>
